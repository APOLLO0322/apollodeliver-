...
export default function AdminListPage() {
  ...
  function expiryLabel(p: Project) {
    if (p.purged_at) return "削除済み（プレビューのみ）";
    if (!p.expires_at) return "自動削除なし";
    const days = Math.ceil((new Date(p.expires_at).getTime() - Date.now()) / 86400000);
    if (days < 0) return "まもなく削除";
    return `あと${days}日で削除`;
  }

  const visible = projects.filter((p) => showPurged || !p.purged_at);
  const purgedCount = projects.filter((p) => p.purged_at).length;
        ...
        <div style={S.listHead}>
          ...
          {purgedCount > 0 && (
            <label style={S.filterLabel}>
              <input type="checkbox" checked={showPurged} onChange={(e) => setShowPurged(e.target.checked)} />
              削除済みも表示（{purgedCount}）
            </label>
          )}
        </div>
              ...
              <div key={p.id} style={S.row}>
                <div style={S.rowMain}>
                  <div style={S.rowTop}>
                    ...
                    {p.purged_at && <span style={S.purgedBadge}>削除済み</span>}
                  </div>
                  <p style={S.meta}>
...
const S: Record<string, CSSProperties> = {
  ...
  listHead: { display: "flex", alignItems: "center", justifyContent: "space-between", margin: "0 0 20px", flexWrap: "wrap", gap: 10 },
  filterLabel: { display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#888", cursor: "pointer" },
  purgedBadge: { fontSize: 10, fontWeight: 500, padding: "2px 7px", borderRadius: 4, background: "#f0f0f0", color: "#999" },
