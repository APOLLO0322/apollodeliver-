...
type PPhoto = { id: string; seq: number; url: string | null; downloadUrl: string | null; filename: string; purged: boolean };
type PVideo = { id: string; seq: number; playUrl: string | null; downloadUrl: string | null; poster: string | null; filename: string; purged: boolean };
    ...
    try {
      for (const file of videos) {
        ...
        await fetch("/api/video-upload-complete", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ projectId: id, seq: urlData.seq, key: urlData.key, filename: file.name, sizeBytes: file.size, microPreview: microV }),
        });
      }
      setVideoProgress(0);
      setVideoState("idle");
          ...
          <section style={S.card}>
            ...
            {pVideos.map((v) => (
              <div key={v.id} style={{ marginBottom: 16 }}>
                {v.purged ? (
                  v.poster ? (
                    <img src={v.poster} alt={v.filename} style={S.pvVideoPoster} />
                  ) : (
                    <div style={S.pvVideoGone}>動画は削除済み</div>
                  )
                ) : (
                  <video src={v.playUrl ?? undefined} controls preload="metadata" style={S.pvVideo} />
                )}
                <div style={S.pvVideoMeta}>
                  <span style={S.pvName}>{v.filename}{v.purged && "（削除済み）"}</span>
                  {!v.purged && v.downloadUrl && (
...
const S: Record<string, CSSProperties> = {
  ...
  pvVideoPoster: { width: "100%", borderRadius: 10, display: "block", filter: "grayscale(0.2)" },
  pvVideoGone: { width: "100%", height: 160, borderRadius: 10, background: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center", color: "#999", fontSize: 13 },
